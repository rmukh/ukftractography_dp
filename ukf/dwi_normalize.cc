...
void dwiNormalize(const Nrrd *raw, Nrrd *&normalized)
{
  ...
  if (raw->space != nrrdSpaceRightAnteriorSuperior &&
      ...
      for (size_t i = 0; i < temp->axis[1].size; i++)
      {
        ...
        for (size_t h = 0; h < temp->axis[0].size; h++)
        {
          ...
          destData[k * temp->axis[2].size * temp->axis[1].size * temp->axis[0].size + j * temp->axis[1].size * temp->axis[0].size + i * temp->axis[0].size + h] =
              (baseline[k * temp->axis[2].size * temp->axis[1].size + j * temp->axis[1].size + i] != 0) ? sourceData[k * normalized->axis[2].size * normalized->axis[1].size * normalized->axis[0].size + j * normalized->axis[1].size * normalized->axis[0].size + i * normalized->axis[0].size + correspondingSourceGradient] / baseline[k * temp->axis[2].size * temp->axis[1].size + j * temp->axis[1].size + i] : 1e-10; // prevent log(0) errors in UnpackTensors( ... )
