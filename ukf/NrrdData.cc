                        ...
                        const bool outputNormalizedDWIData)
{
  ...
  if (outputNormalizedDWIData)
  {
    ...
    std::cout << "Writing normalized signal data to: " << normalizedDataFile << std::endl
              << std::endl;
...
bool NrrdData::LoadSignal(Nrrd *input_nrrd, const bool normalizedDWIData)
{
  ...
  for (unsigned int i = 0; i < gradientCount; ++i)
  {
    ...
    const ukfPrecisionType effectiveBvalue = (std::fabs(gradientNorm - ukfOne) > 1e-4) ? gradientNorm * gradientNorm * bValue : bValue;
