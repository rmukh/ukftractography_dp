...
void Simple3T::F(ukfMatrixType & /** X **/, ukfVectorType /** s **/, const ukfMatrixType & /* &covMatrix */) const {};
                 ...
                 ukfMatrixType &Y) const
{
  ...
  for (unsigned int i = 0; i < X.cols(); ++i)
  {
    ...
    for (int j = 0; j < _signal_dim; ++j)
    {
      ...
      Y(j, i) = std::exp(-b[j] * u.dot(D1 * u)) * weights_on_tensors_[0] + std::exp(-b[j] * u.dot(D2 * u)) * weights_on_tensors_[1] + std::exp(-b[j] * u.dot(D3 * u)) * weights_on_tensors_[2];
    }
  }
}
...
void Simple3T::State2Tensor3T(const State & /* x */, const vec3_t & /* old_m */, vec3_t & /* m1 */, vec3_t & /* m2 */, vec3_t & /* m3 */)
{
}