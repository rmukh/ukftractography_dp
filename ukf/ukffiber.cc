                       ...
                       std::vector<UKFFiber> &fibers)
{
  ...
  for (int i = 0; i < num_branches; i++)
  {
    ...
    if (fiber_index % 2 == 0)
    {
      num_points_on_branch[i] = static_cast<int>(raw_primary[fiber_index + 1].position.size() + raw_branch[i].position.size()) + position_on_fiber - 1;
    }
    else
    {
      num_points_on_branch[i] = static_cast<int>(raw_primary[fiber_index - 1].position.size() + raw_branch[i].position.size()) + position_on_fiber - 1;
  ...
  for (int i = 0; i < num_branches; i++)
  {
    ...
    if (record_fa3)
    {
      fibers[counter].fa3.resize(num_points_on_branch[i]);
    }
    if (record_trace)
    {
    ...
    if (record_w1w2angle)
    {
      fibers[counter].w1w2angle.resize(num_points_on_branch[i]);
    }
    if (record_w1w3angle)
    {
    ...
    for (int j = 0; j < static_cast<int>(branch.position.size()); j++)
    {
      ...
      if (record_fa3)
      {
        fibers[counter].fa3[k] = branch.fa3[j];
      }
      if (record_trace)
      {
      ...
      if (record_w1w2angle)
      {
        fibers[counter].w1w2angle[k] = branch.w1w2angle[j];
      }
      if (record_w1w3angle)
      {
